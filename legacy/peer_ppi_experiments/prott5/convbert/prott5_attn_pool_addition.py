...
import torch
from transformers import T5EncoderModel, TrainingArguments, Trainer, EvalPrediction, T5Tokenizer
...
class PPIAttnAddConvBERT(nn.Module):
    ...
    def forward(self, protein_1, protein_2, attention_mask_1=None, attention_mask_2=None, labels=None):
        protein_1_embed = self._extract_embeddings(protein_1, attention_mask_1)
        protein_2_embed = self._extract_embeddings(protein_2, attention_mask_2)
...
class Dataset:
    ...
    def __getitem__(self, idx):
        ...
        protein_1 = list(re.sub(r"[UZOB]", "X", protein_1))
        protein_2 = list(re.sub(r"[UZOB]", "X", protein_2))
        return {
...
def main():
    ...
    backbone = T5EncoderModel.from_pretrained(ckpt)
    downstream_model = PPIAttnAddConvBERT(backbone)

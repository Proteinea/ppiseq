...
class PPIPoolCatConvBERT(nn.Module):
    ...
    def forward(self, protein_1, protein_2, attention_mask_1=None, attention_mask_2=None, labels=None):
        ...
        pooled_output_1 = global_mean_pooling1d(protein_1_embed, attention_mask_1)
        pooled_output_2 = global_mean_pooling1d(protein_2_embed, attention_mask_2)
...
def main():
    ckpt = "ElnaggarLab/ankh2-ext2"
    print("Checkpoint: ", ckpt)
    tokenizer = AutoTokenizer.from_pretrained(ckpt)
    backbone = T5EncoderModel.from_pretrained(ckpt)
    downstream_model = PPIPoolCatConvBERT(backbone)

    run_name = create_run_name(
        backbone=ckpt,
        setup="convbert_pool_concat",
    )
